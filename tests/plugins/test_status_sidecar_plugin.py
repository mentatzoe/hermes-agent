...
class TestRenderBlock:
    ...
    def test_completed_global_focus_renders_pulse_not_dashboard(self, _isolate_env):
        ss = _load_lib()
        ss.write_status(
            active_project_slug="github/multica",
            active_workspace="/Users/zmll/github/aleph-vault",
            focus_label="nightly sleep consolidation complete",
            focus_state="2026-05-24 note written; 7 nightly-candidate retains queued",
            focus_ref="/Users/zmll/github/aleph-vault/observations/2026-05-24-nightly-sleep.md",
        )
        ss.touch_session(
        ...
        block = ss.render_status_block(
            ttl_seconds=4 * 3600,
            current_session_id="sess-thread",
            current_surface="discord",
            current_chat_name="waffle swarm / #aleph-threads / Heartbeat tweak: make the status sidecar",
            current_chat_type="thread",
            current_thread_id="1507547558434574356",
        )

        assert 'mode="pulse"' in block
        assert "<pulse " in block
        assert "<global_focus " not in block
        assert "nightly sleep consolidation complete" not in block
        assert "nightly-candidate" not in block

    def test_project_only_file_session_hidden_from_pulse(self, _isolate_env):
        ss = _load_lib()
        ss.touch_session(
            session_id="sess-thread",
            surface="discord",
        ...
        ss.touch_session(
            ...
            surface="discord",
            active_project_slug="github/multica",
            last_tool_invocation="patch:.env",
        )
        ...
        assert "sess-file" not in block
        assert "github/multica" not in block
...
class TestPreLlmCallHook:
    ...
    def test_hook_renders_current_thread_metadata_without_global_mix(self, _isolate_env):
        # A new Discord thread may only have a heartbeat/liveness row in the
        # active-session ledger. The current surface should still render as an
        # active chat when gateway chat/thread metadata is present, without
        # borrowing another global workstream.
        plugin = _load_plugin_init()
        result = plugin._on_pre_llm_call(
            session_id="sess-thread",
            user_message="shall we pick this up?",
            conversation_history=[],
            is_first_turn=True,
            model="gpt-5.5",
            platform="discord",
            sender_id="zoe",
            chat_id="1507547558434574356",
            chat_name="waffle swarm / #aleph-threads / Heartbeat tweak: make the status sidecar",
            chat_type="thread",
            thread_id="1507547558434574356",
        )
        assert isinstance(result, dict)
        pulse_line = next(
            line for line in result["context"].splitlines()
            if "<pulse " in line
        )
        assert 'mode="pulse"' in result["context"]
        assert "Heartbeat tweak: make the status sidecar" in pulse_line
        assert 'chat_type="thread"' in pulse_line
        assert 'thread="1507547558434574356"' in pulse_line
        assert 'class="active_chat"' in pulse_line
        assert "<global_focus " not in result["context"]
