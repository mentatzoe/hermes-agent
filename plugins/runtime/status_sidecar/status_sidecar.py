...
def _tool_is_maintenance_observation(tool: Any) -> bool:
    if not isinstance(tool, str):
        return False
    tool = tool.strip().lower()
    return (
        tool in {"cronjob", "patch", "write_file"}
        or tool.startswith("patch:")
        or tool.startswith("write_file:")
    )
...
def _record_has_actionable_global_focus(record: Dict[str, Any]) -> bool:
    """Return true when the status row deserves attention rendering.

    Fresh row timestamps only prove the ledger was touched. They do not prove
    that the semantic focus is still active. Completion-shaped outcomes and
    maintenance/file breadcrumbs should stay behind the pulse unless they also
    carry an explicit action marker.
    """
    focus_text = " ".join(
        str(record.get(key) or "")
        for key in ("focus_label", "focus_state")
    )
    if focus_text and _text_matches_any(focus_text, _ACTIONABLE_MARKERS):
        return True
    if focus_text and _text_matches_any(focus_text, _COMPLETION_MARKERS):
        return False
    if _has_text(record.get("active_kanban_task_id")):
        return True
    if _has_text(record.get("focus_state")) and (
        _has_text(record.get("focus_label"))
        or _has_text(record.get("focus_ref"))
        or _has_text(record.get("active_project_slug"))
    ):
        return True
    return False
...
def _session_is_low_signal(sess: Dict[str, Any]) -> bool:
    ...
    if has_semantic_pointer:
        return False
    # A project slug inferred from a file/cron breadcrumb is provenance, not an
    # active-work claim. Hide it unless another semantic pointer is present.
    if _has_text(sess.get("active_project_slug")) and _tool_is_maintenance_observation(tool):
        return True
    return isinstance(tool, str) and bool(tool.strip()) and not _has_text(sess.get("active_project_slug"))
...
def _compact_render(
    *,
    ...
    """Render pulse by default; escalate only for actionable attention."""
    ...
    if not rendered_sessions and sessions_for_list and not fresh_status:
        # If there is no global status row, keep one meaningful non-current row
        # visible. Low-signal rows stay hidden and counted.
        fallback = [sess for sess in sessions_for_list if not _session_is_low_signal(sess)]
        rendered_sessions = fallback[:1]
        hidden_low_signal = sum(
            1 for sess in sessions_for_list
            if _session_is_low_signal(sess) and sess not in rendered_sessions
        )

    actionable_global = fresh_status and _record_has_actionable_global_focus(record)
    mode = "attention" if actionable_global or rendered_sessions else "pulse"
    lines: List[str] = [
        f'<status mode="{mode}" updated_iso="{_xml_attr(iso, 40)}" age_seconds="{age}" '
        'confidence="pointer" scope="current-work" verify="refs_before_acting">'
    ]
    ...
    has_current_chat_metadata = any(
        isinstance(v, str) and v.strip()
        for v in (current_chat_id, current_chat_name, current_chat_type, current_thread_id)
    )
    if has_current_chat_metadata:
        # Stored activity_class tracks how the ledger row was refreshed. The
        # current rendered surface should classify the Discord thread itself,
        # not promote a tool breadcrumb into the chat's identity.
        activity_class = "active_chat"

    pulse_attrs = _attrs([
        ("surface", surface, 40),
        ("session", surface_id, 80),
        ("chat_type", current_chat_type, 40),
        ("chat", current_chat_name or current_chat_id, MAX_FOCUS_LABEL_CHARS),
        ("chat_id", current_chat_id, 80),
        ("thread", current_thread_id, 80),
        ("class", activity_class, 40),
        ("last_tool", surface_session.get("last_tool_invocation") if surface_session else "", 80),
        ("age_s", age_s, 20),
        ("hidden_low_signal", hidden_low_signal, 20),
    ])
    if pulse_attrs:
        lines.append(f"  <pulse {pulse_attrs} />")

    if actionable_global:
        last_tool = record.get("last_tool_invocation")
        # Maintenance cron activity is useful as provenance, but should not
    ...
    if rendered_sessions:
        ...
        for sess in rendered_sessions:
            ...
            age_s = _fmt_age_seconds(float(seen)) if isinstance(seen, (int, float)) else ""
    ...
    if mode == "attention" and include_events and fresh_status:
    ...
    if mode == "attention" and include_digest and fresh_status:
